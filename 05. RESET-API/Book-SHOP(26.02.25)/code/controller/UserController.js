...
const join = (req, res) => {
  ...
  // 회원가입 시 비밀번호를 안호화해서 암호화된 비밀번호와, salt 값을 같이 저장
  // 비밀번호 암호화
...
const login = (req, res) => {
  ...
  conn.query(sql, email, (err, results) => {
    ...
    // salt 값을 꺼내서 비밀번호 암호화 해보고 => 디비 비밀번호와 비교
...
const passwordResetRequest = (req, res) => {
  ...
  // email이 없으면 400
  if (!email) {
    return res.status(StatusCodes.BAD_REQUEST).json({ message: 'email 필요' });
  }
...
const passwordReset = (req, res) => {
    ...
    conn.query(sql, values, (err, results) => {
            if (err) {
                console.log(err);
                return res.sendStatus(StatusCodes.BAD_REQUEST).end();
            }

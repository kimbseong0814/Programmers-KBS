...
📌 Map의 forEach는 (value, key) 순서로 전달된다.

2️⃣ forEach vs map 차이
핵심 차이
구분	forEach	map
반환값	❌ 없음	✅ 새 배열
목적	순회·출력	데이터 변환
...
const forEachResult = arr.forEach(v => v * 2);
const mapResult = arr.map(v => v * 2);

console.log(forEachResult); // undefined
console.log(mapResult);    // [2, 4, 6]


📌 데이터를 가공해서 새 결과가 필요하면 map을 사용해야 한다.

3️⃣ delete (Map 삭제)
개념

delete는 Map에서 특정 key에 해당하는 데이터를 제거하는 메서드이다.
...
📌 PUT은 POST와 달리 기존 데이터가 전제다.

5️⃣ 리팩토링이란?
정의

리팩토링(Refactoring)은
기능은 그대로 유지하면서 코드 구조를 개선하는 작업이다.
